...
class CBNode(Node):
    ...
    def action_prog_widen(self,coef={'ka':0.5, 'alpha_a':0.5, 'c':0.5}, mode='max'):
        ...
        if len(self.children) <= ka*self.visits**alpha_a:
            ...
            next_state, _, _, _ = self.state.step(new_action)
    ...
    def add_child(self, state, action):
        ...
        if child is None:
            child = CBNode(action,state,self.depth+1,self)
            self.children.append(child)
            child._set_default_actions()
        return child
...
def find_new_belief_root(
 current_state, previous_action, previous_root
) -> CBNode:
