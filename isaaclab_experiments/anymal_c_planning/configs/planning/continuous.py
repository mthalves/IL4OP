...
class OnlinePlanning(ManagerTermBase):
    ...
    available_problems = {
        'inspection.continuous': 'ContinuousInspectionProblem',
    }
    ...
    def get_problem(self, env, problem_name, args):
        if problem_name is None:
            ...
            self.problem_name = 'inspection.continuous'
        else:
    ...
    def import_problem(self,problem_name):
        ...
        path = 'isaaclab_experiments.src.problems'
        ...
        try:
            ...
            print('Loading default planning method: inspection.continuous')
            method = getattr(module, self.available_problems['inspection.continuous'])
        return method

    def get_planner(self, planning_method_name, planning_method_args):
        if planning_method_name is None:
            print(f"Default planning method not implemented. Exiting.")
            exit(1)
        else:
    ...
    def __call__(self, env: ManagerBasedEnv, env_ids: torch.Tensor | None,
        ...
        agent = {
            ...
            'radius': current_state.map.robot_radius_w,
        ...
        cmd.pos_command_w[:, 2] = robot.data.default_root_state[0, 2] # fixed height
